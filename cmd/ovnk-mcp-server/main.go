...
func setupOffline(server *mcp.Server) {
	...
	mustGatherServer := mustgathermcp.NewMCPServer()
	if mustGatherServer == nil {
		log.Println("Failed to create Must Gather MCP server, will not be able to use must gather tools")
		return
	}
