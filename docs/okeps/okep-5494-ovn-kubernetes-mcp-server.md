...
The MCP server parameters can be tuned using a configuration file which is
provided at startup of the MCP server. This allows customization of server
behavior, timeout values, and other operational parameters without requiring
code changes. The configuration also supports disabling specific tools that
aren't supported in certain deployment stacks or environments, which helps
with scalability by reducing context size sent to the LLM and improves tool
selection accuracy.
