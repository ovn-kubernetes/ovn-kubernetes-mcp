...
and leveraging the backend Model's (Claude Sonnet 4, Gemini 2.5 Pro, GPT-5, etc.)
...
1. An end-user can use any MCP Client to start a troubleshooting session via
   ...
   MCP Servers) and gathers their available tools and presents this information
...
2. LLM will be able use its intelligence and analyze the end-user query and
   choose the appropriate tools. Example: Using Claude Sonnet 4 as your LLM
   model
...
* `kubectl exec` and `kubectl debug node` operations require elevated
  Kubernetes permissions - specifically `pods/exec` on the target
  namespace, and `nodes/proxy` (plus `pods/exec` against the spawned
  debug pod) for `kubectl debug node` - not necessarily full
  cluster-admin. Even so, these are powerful primitives and we avoid
  exposing a generic exec tool in the K8s layer; prefer direct
  Kubernetes API reads (`get`, `list`, `logs`, `events`).
...
and pivoted towards opting into a more secure approach of adding a
...
Phase-1 of the MCP Server targets OVN-Kubernetes interconnect mode
(the default in current releases), since that is where the per-node
NB/SB/OVS databases described below actually live. The tool surface
itself is generic and may work in non-interconnect deployments, but
they are out of scope for this OKEP.
...
* Performance/Scalability: MCPs are a relatively new concept. So aspects like
  how many tools could we expose per server and up to what point it scales etc.
