...
// NewMCPServer creates a new MustGatherMCPServer. It will return nil if the omc client
// cannot be created.
func NewMCPServer() *MustGatherMCPServer {
	omcClient, err := omcclient.NewOmcClient()
	if err != nil {
		log.Printf("Failed to create omc client: %v", err)
		return nil
	}
