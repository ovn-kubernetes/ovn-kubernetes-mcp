...
func TestParseOutput(t *testing.T) {
	tests := []struct {
		...
		{
			name:           "parse ovn-nbctl ls-list output",
			testdataFile:   "ovn-nbctl-ls-list.txt",
			expectedLines:  5,
			checkFirstLine: `7a8b9c0d-1234-5678-9abc-def012345678 (ovn-worker)`,
			checkLastLine:  `efgh5678-ijkl-mnop-qrst-uvwxyz987654 (ext_ovn-control-plane)`,
		},
		{
			name:           "parse ovn-nbctl acl-list output",
			testdataFile:   "ovn-nbctl-acl-list.txt",
			expectedLines:  9,
			checkFirstLine: `from-lport  1001 (inport == @a16982411286042166782 && ip4) allow-related`,
			checkLastLine:  `to-lport     500 (outport == @pg_default_deny_ingress) drop`,
		},
		{
		...
		{
			name:           "parse ovn-nbctl lr-nat-list output",
			testdataFile:   "ovn-nbctl-lr-nat-list.txt",
			expectedLines:  5,
			checkFirstLine: `TYPE             EXTERNAL_IP        EXTERNAL_PORT    LOGICAL_IP          EXTERNAL_MAC         LOGICAL_PORT`,
			checkLastLine:  `dnat_and_snat    172.18.0.101                        10.244.1.6                               default_web-server-6c8d9e0f1a-xyz34`,
		},
	}
...
func TestFilterLinesWithOVNOutput(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			testdataFile:  "ovn-nbctl-ls-list.txt",
			pattern:       "ovn-worker",
			expectedCount: 2, // ovn-worker and ext_ovn-worker
			shouldContain: "ovn-worker",
		},
		{
			name:          "filter switches starting with ext_",
			testdataFile:  "ovn-nbctl-ls-list.txt",
			pattern:       `\(ext_`,
			expectedCount: 2,
			shouldContain: "ext_",
		...
		{
			name:          "filter ACLs for specific IP range",
			testdataFile:  "ovn-nbctl-acl-list.txt",
			pattern:       "10\\.244\\.0\\.0/16",
			expectedCount: 2,
			shouldContain: "10.244.0.0/16",
		},
		{
...
// TestValidateSwitchName tests logical switch name validation.
func TestValidateSwitchName(t *testing.T) {
	tests := []struct {
		...
		{"empty allowed", "", false},
		{"node switch", "ovn-worker", false},
		{"join switch", "join", false},
		{"external switch", "ext_ovn-worker", false},
		{"injection", "switch;drop", true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("validateSwitchName(%q) error = %v, wantErr %v", tt.switchName, err, tt.wantErr)
			}
		})
	}
}

// TestValidateEntityName tests entity (switch or port group) name validation.
func TestValidateEntityName(t *testing.T) {
	tests := []struct {
		...
		{"empty allowed", "", false},
		{"logical switch", "ovn-worker", false},
		{"port group", "pg_default_deny", false},
		{"port group with namespace", "a16982411286042166782", false},
		{"injection", "entity;drop", true},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("validateEntityName(%q) error = %v, wantErr %v", tt.entityName, err, tt.wantErr)
			}
		})
	}
}

// TestValidateRouterName tests logical router name validation.
func TestValidateRouterName(t *testing.T) {
	tests := []struct {
		name       string
		routerName string
		wantErr    bool
		...
		{"empty allowed", "", false},
		{"cluster router", "ovn_cluster_router", false},
		{"gateway router", "GR_ovn-worker", false},
		{"gateway router control plane", "GR_ovn-control-plane", false},
		{"injection", "router;drop", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := validateRouterName(tt.routerName)
			if (err != nil) != tt.wantErr {
				t.Errorf("validateRouterName(%q) error = %v, wantErr %v", tt.routerName, err, tt.wantErr)
			}
		})
	}
}
